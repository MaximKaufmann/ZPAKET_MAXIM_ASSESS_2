class ZCL_ZMAXIM_OB_DPC definition
  public
...
protected section.
  ...
  methods ZTEACH_CONTACTSE_CREATE_ENTITY
    importing
    ...
    exporting
      !ER_ENTITY type ZCL_ZMAXIM_OB_MPC=>TS_ZTEACH_CONTACT
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods ZTEACH_CONTACTSE_DELETE_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
    ...
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods ZTEACH_CONTACTSE_GET_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
    ...
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods ZTEACH_CONTACTSE_GET_ENTITYSET
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
    ...
    exporting
      !ET_ENTITYSET type ZCL_ZMAXIM_OB_MPC=>TT_ZTEACH_CONTACT
      !ES_RESPONSE_CONTEXT type /IWBEP/IF_MGW_APPL_SRV_RUNTIME=>TY_S_MGW_RESPONSE_CONTEXT
    raising
      /IWBEP/CX_MGW_BUSI_EXCEPTION
      /IWBEP/CX_MGW_TECH_EXCEPTION .
  methods ZTEACH_CONTACTSE_UPDATE_ENTITY
    importing
      !IV_ENTITY_NAME type STRING
      !IV_ENTITY_SET_NAME type STRING
      !IV_SOURCE_NAME type STRING
...
CLASS ZCL_ZMAXIM_OB_DPC IMPLEMENTATION.
...
*&* This class has been generated on 23.03.2023 10:48:30 in client 500
*&*
*&*       WARNING--> NEVER MODIFY THIS CLASS <--WARNING
*&*   If you want to change the DPC implementation, use the
*&*   generated methods inside the DPC provider subclass - ZCL_ZMAXIM_OB_DPC_EXT
*&-----------------------------------------------------------------------------------------------*

 DATA zteach_contactse_create_entity TYPE zcl_zmaxim_ob_mpc=>ts_zteach_contact.
...
*-------------------------------------------------------------------------*
*             EntitySet -  ZTEACH_CONTACTSet
*-------------------------------------------------------------------------*
     WHEN 'ZTEACH_CONTACTSet'.
*     Call the entity set generated method
    zteach_contactse_create_entity(
         EXPORTING iv_entity_name     = iv_entity_name
       	 ...
       	 IMPORTING er_entity          = zteach_contactse_create_entity
    ).
*     Send specific entity data to the caller interfaces
    copy_data_to_ref(
      EXPORTING
        is_data = zteach_contactse_create_entity
      CHANGING
        cr_data = er_entity
   ).
...
*-------------------------------------------------------------------------*
      when 'ZTEACH_CONTACTSet'.
*     Call the entity set generated method
     zteach_contactse_delete_entity(
          EXPORTING iv_entity_name     = iv_entity_name
                    iv_entity_set_name = iv_entity_set_name
                    iv_source_name     = iv_source_name
                    it_key_tab         = it_key_tab
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA zteach_contactse_get_entity TYPE zcl_zmaxim_ob_mpc=>ts_zteach_contact.
 DATA lv_entityset_name TYPE string.
...
*     Call the entity set generated method
          zteach_contactse_get_entity(
               EXPORTING iv_entity_name     = iv_entity_name
                         ...
                         it_navigation_path = it_navigation_path
                         io_tech_request_context = io_tech_request_context
             	 IMPORTING er_entity          = zteach_contactse_get_entity
                         es_response_context = es_response_context
          ).

        IF zteach_contactse_get_entity IS NOT INITIAL.
*     Send specific entity data to the caller interface
          copy_data_to_ref(
            EXPORTING
              is_data = zteach_contactse_get_entity
            CHANGING
              cr_data = er_entity
          ).
        ELSE.
...
*&*   generated methods inside the DPC provider subclass - ZCL_ZMAXIM_OB_DPC_EXT
*&-----------------------------------------------------------------------------------------------*
 DATA zteach_contactse_get_entityset TYPE zcl_zmaxim_ob_mpc=>tt_zteach_contact.
 DATA lv_entityset_name TYPE string.
...
*-------------------------------------------------------------------------*
   WHEN 'ZTEACH_CONTACTSet'.
*     Call the entity set generated method
      zteach_contactse_get_entityset(
        EXPORTING
         iv_entity_name = iv_entity_name
         iv_entity_set_name = iv_entity_set_name
         iv_source_name = iv_source_name
         ...
         io_tech_request_context = io_tech_request_context
       IMPORTING
         et_entityset = zteach_contactse_get_entityset
         es_response_context = es_response_context
       ).
*     Send specific entity data to the caller interface
      copy_data_to_ref(
        EXPORTING
          is_data = zteach_contactse_get_entityset
        CHANGING
...
*&-----------------------------------------------------------------------------------------------*
 ...
 DATA zteach_contactse_update_entity TYPE zcl_zmaxim_ob_mpc=>ts_zteach_contact.
 DATA lv_entityset_name TYPE string.
 DATA lr_entity TYPE REF TO data. "#EC NEEDED
...
*-------------------------------------------------------------------------*
      WHEN 'ZTEACH_CONTACTSet'.
*     Call the entity set generated method
          zteach_contactse_update_entity(
               EXPORTING iv_entity_name     = iv_entity_name
                         iv_entity_set_name = iv_entity_set_name
                         iv_source_name     = iv_source_name
                         io_data_provider   = io_data_provider
                         ...
                         it_navigation_path = it_navigation_path
                         io_tech_request_context = io_tech_request_context
             	 IMPORTING er_entity          = zteach_contactse_update_entity
          ).
       IF zteach_contactse_update_entity IS NOT INITIAL.
*     Send specific entity data to the caller interface
          copy_data_to_ref(
            EXPORTING
              is_data = zteach_contactse_update_entity
            CHANGING
              cr_data = er_entity
          ).
        ELSE.
...
* Unit test injection
  ...
  method IF_SADL_GW_DPC_UTIL~GET_DPC.
    TYPES ty_ZTEACH_CONTACT_1 TYPE zteach_contact ##NEEDED. " reference for where-used list

    DATA(lv_sadl_xml) =
               |<?xml version="1.0" encoding="utf-16"?>| &
               |<sadl:definition xmlns:sadl="http://sap.com/sap.nw.f.sadl" syntaxVersion="V2" >| &
               | <sadl:dataSource type="DDIC" name="ZTEACH_CONTACTSet" binding="ZTEACH_CONTACT" />| &
               |<sadl:resultSet>| &
               |<sadl:structure name="ZTEACH_CONTACTSet" dataSource="ZTEACH_CONTACTSet" maxEditMode="RO" >| &
               | <sadl:query name="EntitySetDefault">| &
               | </sadl:query>| &
               | <sadl:attribute name="ID" binding="ID" isKey="FALSE" />| &
               | <sadl:attribute name="VORNAME" binding="VORNAME" isKey="FALSE" />| &
               | <sadl:attribute name="NACHNAME" binding="NACHNAME" isKey="FALSE" />| &
               | <sadl:attribute name="TELEFONNUMMER" binding="TELEFONNUMMER" isKey="FALSE" />| &
               |</sadl:structure>| &
               |</sadl:resultSet>| &
               |</sadl:definition>| .
    ro_dpc = cl_sadl_gw_dpc_factory=>create_for_sadl( iv_sadl_xml   = lv_sadl_xml
               iv_timestamp         = 20230323094834
               iv_uuid              = 'ZMAXIM_OB'
               io_query_control     = me
...
RETURN.
  ...
  method ZTEACH_CONTACTSE_CREATE_ENTITY.
  ...
  method ZTEACH_CONTACTSE_UPDATE_ENTITY.
